 ...
 */
package com.liquibase.dal.service;
...
//            	 exceptionHandlerService.writeLog(e);
    ...
    public <T> List<T> findEntityWithCriteria(Class classObj,Map<String,Object> vars){
        LocalSessionManager manager = new LocalSessionManager();
        Session session = manager.getSession(sessionFactory);
        Criteria cr = session.createCriteria(classObj);
        Iterator itr = vars.keySet().iterator();
        while(itr.hasNext()){
            String key = (String) itr.next();
            if(vars.get(key) instanceof Object[]){
                System.out.println("in list...");
                cr.add(Restrictions.in(key, (Object[]) vars.get(key)));
            }else{
                System.out.println("in eq...");
                cr.add(Restrictions.eq(key, vars.get(key)));
            }
